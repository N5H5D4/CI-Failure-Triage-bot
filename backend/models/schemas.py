...
class AIAnalysisOutput(BaseModel):
    failure_category: str = Field(description="One of: flaky_test, test_failure, dependency_issue, syntax_error, configuration_error, infrastructure_timeout, unknown")
    confidence_score: float = Field(ge=0.0, le=1.0, description="Confidence between 0.0 and 1.0")
    root_cause: str = Field(description="Plain English or Vietnamese explanation referencing offending log lines")
    suggested_fix: str = Field(description="Concrete actionable remediation fix")
