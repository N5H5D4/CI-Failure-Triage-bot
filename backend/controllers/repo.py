...
async def get_setup_info(request: Request):
    ...
    ai_key = os.getenv("CLAUDE_API_KEY") or os.getenv("GEMINI_API_KEY", "")
...
async def connect_repository(repo_in: RepositoryConfigCreate, db: Session = Depends(get_db)):
    ...
    existing = db.query(RepositoryConfig).filter_by(owner=owner, name=name).first()
    if existing:
        existing.webhook_secret_ref = repo_in.webhook_secret or os.getenv("WEBHOOK_SECRET", "")
    ...
    new_repo = RepositoryConfig(
        owner=owner,
        name=name,
        webhook_secret_ref=repo_in.webhook_secret or os.getenv("WEBHOOK_SECRET", ""),
...
async def test_repo_connection(payload: dict):
    ...
    if secret:
        sig = "sha256=" + hmac.new(secret.encode("utf-8"), sample_payload, hashlib.sha256).hexdigest()
    else:
